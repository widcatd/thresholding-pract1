...
def conservative_smoothing(direccion,w_size,cf):#segundo parametro debe ser impar mayor que 1
    ...
    if img is not None:
        ...
        for i in range(img.shape[0]):
            for j in range(img.shape[1]):
                ...
                #sacamos el promedio
                temporal=submatrix[(submatrix.shape[0]//2)+1][(submatrix.shape[1]//2)+1]
                submatrix[(submatrix.shape[0]//2)+1][(submatrix.shape[1]//2)+1]=0
                submatrix[(submatrix.shape[0]//2)+1][(submatrix.shape[1]//2)+1]=np.amax(submatrix)
                minimo=np.amin(submatrix)
                maximo=np.amax(submatrix)
                if minimo>=temporal:
                    out[i][j]=minimo
                elif maximo<=temporal:
                    out[i][j]=maximo
                else:
                    out[i][j]=temporal
